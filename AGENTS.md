...
- **Validate IDs before use.** Route converters use `<int(signed=True):...>` where negative IDs are meaningful (query samples have negative `sample_id`). Check `client.isSampleId` / `isFamilyId` / `isFunctionId` before acting on user-supplied IDs, unless the call you need answers None for an unknown id itself (`getSampleById`, `getFunctionsBySampleId`, `getMatchFunctionVs`): then that None is the check, and a probe first is a second round trip that learns nothing (#207). The client answers None for a backend error too, so the branch that handles it must not assume the id was wrong.

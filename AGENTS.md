...
Requires **Python 3.11 or newer** (`pyproject.toml` sets `requires-python = ">=3.11"`) - inherited from mcrit, which has declared it since v1.5.0; the reference deployment (`docker-mcrit`) runs **Python 3.12**. Target 3.11/3.12 for anything new.

```bash
make init   # pip install -e ".[dev]": the package, plus pytest/pytest-cov/ruff at the versions CI pins
```

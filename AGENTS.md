...
- **`|tojson` ends at the script tag, and a JS sink can be further on.** `static/autocomplete.js` builds each type-ahead suggestion as an HTML string and assigns it to `innerHTML`, so a family name that *is* markup executed there even though `|tojson` had delivered it as a perfectly correct JS string. Two sinks in that one string, not one: the button's text, and the `data-label`/`data-value` attributes the same value is interpolated into — an attribute break needs no angle bracket, so escaping only `<`/`>` would look like a fix and not be one. It is vendored and not ours to patch, so family names reach it one way only: `templates/js/ac_family_names.html` fetches them from `explore.family_names`, which builds the items with `mcritweb.autocomplete.autocomplete_items` and so HTML-escapes label and value on the way in. The `submit_or_query_dropzone` macro includes that same partial for its family field; no page embeds the names in its source (#192). `tests/testAutocompleteEscaping.py` fails on a `new Autocomplete(`/`setData(` fed anything but an empty literal or the endpoint's `data.suggestions`, and accepts the latter only in that partial and only while nothing else there touches it. When a value crosses into JavaScript, ask what the script does with it next — `|tojson` is the transport, not the whole answer.

...
function highlightUERs(UERtype){
  ...
  function loadWithDotGraphAndFunctionId(function_id) {
    isTraceSupplied = false;
    // Send request for loop information; load when ready
     d3.xhr("../fetchDotGraph/" + function_id).header("Content-Type", "text/plain")
  ...
  function showGraph(isTraceSupplied) {
    ...
    var bounds = d3.select("#graphContainer").node().getBoundingClientRect();
  ...
  d3.select("#enableBrush").on("change", function() {
    ...
    if(!isTraceSupplied){
      ...
      // svg.node().style.height = text_box_height; 
      var bBox = svg.node().getBBox();
      console.log('XxY', bBox.x + 'x' + bBox.y);
      console.log('size', bBox.width + 'x' + bBox.height);
      var bounds = d3.select("#xcfg_text_right").node().getBoundingClientRect();
      console.log('XxY', bounds.x + 'x' + bounds.y);

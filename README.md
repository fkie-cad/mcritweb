...
## Version History
 * unreleased: **Requires mcrit 1.9.0**, in both `requirements.txt` and `setup.py`, so the install has to be re-run and a container image rebuilt. The server page can start the three repairs mcrit 1.9.0 added - the MinHashes an older smda escaper produced, the family statistics, and the PicBlockHash index - which `/status` could report but only the backend could start, and each one's report renders on its job page. They need a 1.9.0 backend too; an older one refuses them, and a refused maintenance job, old or new, is now a message on the server page instead of a server error.
